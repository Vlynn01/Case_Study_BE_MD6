...
public interface IUserApplyRepo extends CrudRepository<UserApply,Integer> {
    @Query(nativeQuery = true,value = "select * from Case_Study_MD6.user_apply where (imgcv=:imgcv or  mail_cv=:mail_cv or  numbercv=:numbercv) and  app_user_id=:idAppUser and post_enterprise_id_post_enterprise=:idPost")
    ...
    @Modifying
    @Transactional
    @Query(nativeQuery = true,value = "update Case_Study_MD6.user_apply set status_confirm=1 where id_user_apply=:id")
    void updateStatusConfirmUserApply(@Param("id") int id);

    @Query(nativeQuery = true,value = "select * from Case_Study_MD6.user_apply where post_enterprise_id_post_enterprise=:id")
    List<UserApply> listUserApplyByIdPost(@Param("id") int id);

    @Query(nativeQuery = true,value = "select post_enterprise_id_post_enterprise from Case_Study_MD6.user_apply where app_user_id=:id")
    List<Integer> listIdPostByIdUserApply(@Param("id") int id);

    @Query(nativeQuery = true, value = "select * from Case_Study_MD6.user_apply where app_user_id=:id")
