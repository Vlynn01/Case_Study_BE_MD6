...
public interface IPostEnterpriseRepo extends PagingAndSortingRepository<PostEnterprise, Integer> {
    @Query(nativeQuery = true,value = "select  * from case_study_be_md6.post_enterprise")
    List<PostEnterprise> getAll(Pageable pageable);
    @Query(nativeQuery = true, value = "select  * from case_study_be_md6.post_enterprise where id_post_enterprise=:id")
    List<PostEnterprise> findAllById(@Param("id") int id);

    @Query(nativeQuery = true, value = "select  * from case_study_be_md6.post_enterprise where enterprise_id_enterprise=:id")
...
//                    where app_user_id = 3)
    ...
    @Query(nativeQuery = true, value = "select  * from case_study_be_md6.post_enterprise where enterprise_id_enterprise=:id and regime_id_regime=1")
    List<PostEnterprise> listPostVipByEnterprise(@Param("id") int id);

    @Query(nativeQuery = true, value = "select  * from case_study_be_md6.post_enterprise where enterprise_id_enterprise=:id and regime_id_regime=2")
...
//     số lượng apply theo post
    @Query(nativeQuery = true,value = "select quantity_apply_post from case_study_be_md6.post_enterprise where  id_post_enterprise=:id ")
...
// lấy và chỉnh sửa  điểm đề xuất của bài post theo id
    @Query(nativeQuery = true,value = "select  priority_post_enterprise from case_study_be_md6.post_enterprise where id_post_enterprise=:id")
...
//    Thực hiện xóa bài viết khi hết hạn !
    @Modifying
    @Transactional
    @Query(nativeQuery = true,value = "delete  from case_study_be_md6.post_enterprise where expiration_date_post_enterprise=:date ")
    void  deletePostExpired(@Param("date") String date);

//    lấy tất cả bài đăng của ngày hiện tại
    @Query(nativeQuery = true,value = "select * from case_study_be_md6.post_enterprise where expiration_date_post_enterprise=:date limit 1")
